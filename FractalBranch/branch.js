function Branch(begin, end, thickness) {
  ...
  this.growBranchRight = function() {
    ...
    dir.rotate(angleRandomizer(minAngle, maxAngle) * Math.PI/180);  //randomize angle
    dir.mult(gRatio * lengthRandomizer(minLengthMod,maxLengthMod)); //shorten branches
  ...
  this.growBranchLeft = function() {
    var dir = p5.Vector.sub(this.end,this.begin);                   //append it to the existing branch
    dir.rotate(-angleRandomizer(minAngle,maxAngle) * Math.PI/180);  //randomize angle, the in other direction
    dir.mult(gRatio * lengthRandomizer(minLengthMod,maxLengthMod)); //shorten branches
    var newEnd = p5.Vector.add(this.end,dir);                       //save new endpoint in var
